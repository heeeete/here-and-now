...
### 1. 지도 이동 시 API 호출 최적화 (Debounce 적용)

- **문제**: 지도를 조금만 움직여도 `idle` 이벤트가 수없이 발생하여 서버에 과부하를 주는 문제가 있었습니다.
- **해결**: `useMemo`와 커스텀 `debounce` 유틸리티를 조합해 300ms 지연을 두어 지도가 완전히 멈춘 뒤에만 API가 호출되도록 제어했습니다.
- **효과**: 불필요한 API 요청을 줄이고 매끄러운 사용자 경험을 제공했습니다.

### 2. "나도 여기 기록하기": 사용자 피드백 반영

- **문제**: 이미 기록이 남겨진 좌표에는 다른 사람이 기록을 남길 수 없다는 피드백이 있었습니다.
- **해결**: 기존 마커 클릭 시 나타나는 상세 UI에 '나도 여기 기록하기' 버튼을 추가하고, 동일 위치에 여러 기록이 존재할 수 있도록 로직을 수정했습니다.
