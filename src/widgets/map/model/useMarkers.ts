...
export const useMarkers = (
  ...
  const updateClickMarker = useCallback((lat: number, lng: number | null) => {
    if (!map) return;
  ...
  // 기록 마커 및 클러스터 렌더링
  const renderRecords = useCallback((records: Record[]) => {
    if (!map) return;
    ...
    // 클러스터링 계산 (픽셀 거리 기반)
    const clusters: { center: Record; centerPixel: naver.maps.Point; count: number; members: Record[] }[] = [];
